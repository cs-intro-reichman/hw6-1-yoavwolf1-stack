...
public class Runigram {
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		...
		if(image.length == 0 || image[0].length == 0) return new Color[0][0];
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		double ratioI = (double)image.length / height;
		double ratioJ = (double)image[0].length / width;
		Color[][] scaled = new Color[height][width];
		for(int i = 0; i < height; i++)
			for(int j = 0; j < width; j++) {
				scaled[i][j] = image[(int)(i * ratioI)][(int)(j * ratioJ)];
			}
	...
	public static Color[][] blend(Color[][] image1, Color[][] image2, double alpha) {
		...
		if(image1.length == 0 || image1[0].length == 0) return new Color[0][0];
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		...
		if(source.length == 0 || source[0].length == 0 || target.length == 0 || target[0].length == 0) return;

		int rows = source.length;
		int cols = source[0].length;
		double morphFactor = 1.0 / n;

		Color[][] targetS = scaled(target, cols, rows);
		Color[][] blended = new Color[rows][cols];

		for(int i = 0; i <= n; i++) {
			blended = blend(source, targetS, 1 - morphFactor * i);
